...
def add_scatter_borders(ax):
    for child in ax.get_children()[::-1]:
        if not isinstance(child, matplotlib.collections.PathCollection): continue
...
def marker_violins(adata, genes, cluster_key, palette, cluster_name="Cluster", filename=None, figdir=None):
    L = len(genes)
    fig, axs = plt.subplots(L, 1, figsize=(4, 0.32*L))
    violin_data = pd.DataFrame(adata[:, genes].X.toarray(), 
                               columns=genes)
    violin_data[cluster_name] = adata.obs[cluster_key].values

    for k, (ax, gene) in enumerate(zip(axs.flat, genes)):
        sns.violinplot(
            data=violin_data, x=cluster_name, y=gene, 
            palette=palette, ax=ax,
            inner=None, linewidth=0.4, scale="width", cut=0
        )
        ax.set_ylabel(gene, rotation=0, ha="right", va="top", size="small", style="italic")
        ax.set_yticks([])
...
def genes_umi_violins(adata, cluster_key, palette, filename=None, figdir=None, ylims=[50000, 10000]):
    ...
    for k, (ax, obs, ylim, ytick) in enumerate(zip(axs.flat, obs_names, ylims, yticks)):
        ...
        sns.violinplot(
            data=violin_data, x="Cluster", y=obs, 
            palette=palette, ax=ax,
            inner=None, linewidth=0.4, scale="width", cut=0
        )
...
def get_matrix_data2(adata, markers, cluster_key, palette):
    grpd = markers.groupby(cluster_key).head(25)
    #grpd = grpd[~grpd.gene_name.duplicated()]

    genes = pd.Index(["Snap25", "Syp", "Tubb3", "Elavl2", "Slc17a6", "Slc32a1", "Gad1", "Gad2",
                      "Aqp4", "Gfap", "Rax", "Dcn", "Rgs5", "C1qa", "Tmem119", "Pdgfra", "Fyn", "Mobp", "Pecam1"])
    n_clusters = len(adata.obs[cluster_key].cat.categories)
    data = np.zeros((len(genes), n_clusters))
    tmp = adata[:,genes].X.toarray()
    for i, gene in enumerate(genes):
        for j, clusterj in enumerate(adata.obs[cluster_key].cat.categories):
            cell_ind = adata.obs[cluster_key].isin([clusterj])
            data[i, j] = tmp[cell_ind, i].mean()
...
def fix_heatmap_colorbar(cg, adata, heatmap_data, cmap):
    ...
    sns.heatmap(heatmap_data, vmin=-3, vmax=3, cmap=cmap, ax=_ax, 
                cbar_ax=new_cbar_ax, cbar_kws={"orientation": "horizontal"})
    _ax.remove()
...
def get_sample_proportions(adata,
                            cluster_key="cluster_final",
                            sample_key="replicate",
                            drop_values=None):
...
def plot_cluster_proportions(cluster_props, 
    ...
    ax.legend(bbox_to_anchor=(1.01, 1), frameon=False, title="Cluster", ncol=2, columnspacing=1)
    sns.despine(fig, ax)
...
def plot_sample_proportions(cluster_props, 
                             cluster_palette=None,
                             xlabel_rotation=0): 
    ...
    ax.legend(bbox_to_anchor=(1.01, 1), frameon=False, title="Sample", ncol=1, columnspacing=1)
    sns.despine(fig, ax)
    ax.tick_params(axis="x", rotation=xlabel_rotation)
...
def plot_celltype_proportions(
    ...
    ax.legend(bbox_to_anchor=(0.5, 0.05), loc="upper center", 
              handletextpad=0.2, columnspacing=1, frameon=False, title="", ncol=10)
    fig.tight_layout()
    return fig
...
def cluster_denodrogram(adata, groupby, palette, figdir, highly_variable=True, colored=False, prefix="neuronal"):
    inds = \
        (adata.var.n_cells > 40) & (~adata.var.mitochondrial) & (~adata.var.ribosomal) &\
        (~adata.var.hemoglobin) & (~adata.var.sex_linked) & (~adata.var.stress) & (~adata.var.cell_cycle)
    if highly_variable:
        inds &= adata.var.highly_variable
    genes = adata.var_names[inds]
    
    print(f"Using {len(genes)} genes")
    categories, tidy_obs = _prepare_dataframe(
        adata, genes, groupby=groupby, use_raw=False
    )
    mean_obs = tidy_obs.groupby(level=0).mean()
    
    print("Data prepared...")
    Z = linkage(mean_obs, method="average", metric="correlation", optimal_ordering=True)
    cluster_order = leaves_list(Z)
    clusters_ordered = categories[cluster_order]
    
    fig, ax = plt.subplots(figsize=(3, 1))
    with plt.rc_context({"lines.linewidth": 0.7}):
        colors = ["k"]*100
        if colored:
            colors = ["k"]*20 + ["red"]*12 + ["k"]*100
        
        dendrogram(
            Z, above_threshold_color="k",
            link_color_func=lambda x: colors[x],
            color_threshold=0.42*Z[:,2].max(), distance_sort=True, ax=ax, 
            labels=categories
        )
        ...
        figname = (
            f"{prefix}-cluster-dendrogram{['','_highly-variable'][highly_variable]}"
            f"{['','_colored'][colored]}"
        )
        save_figure(fig, figdir, figname)
